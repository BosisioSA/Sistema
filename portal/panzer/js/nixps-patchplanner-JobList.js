...
(function( $ ) {
    ...
    $.widget("nixps-patchplanner.JobList", {

        options: {

        	patchInfo: null,

        	filterFullyPatched: true,
        ...
        _create: function() {
        	...
        	$('<table>').css({
        		width: "100%"
        	}).append($("<tbody>").addClass('joblist')).appendTo(this.element);
		...
		_draw: function() {
			var jobList = this.element.find('.joblist');
            var doSearch = typeof this.options.search === 'string' && this.options.search.length > 0;
			jobList.empty();

			var filterMOM = this.options.filterMOM;
			var patchInfo = this.options.patchInfo;
			var jobs = [];
			if (filterMOM === true) {
				jobs = patchInfo.getJobsWithMOM();
			}
			else {
				jobs = patchInfo.getJobs();
			}

			for(var i = 0; i < jobs.length; i++) {
				var job = jobs[i],
					placedCount = patchInfo.getPlacedPatchCountForJob(job),
					totalCount = patchInfo.getPatchCountForJob(job);

                // Skip jobs that don't match the search string
                if (doSearch === true && job.indexOf(this.options.search) < 0) {
                    continue;
                }
				...
				row.append($('<td>').text(job).css({
					'white-space': 'nowrap'
				})).append($('<td>').text(placedCount + " of " + totalCount).css({
					'text-align': 'right',
					'width': '99%'
				}));
				...
				row.appendTo(jobList).data('job', job);
			}
		},
		...
		_expandClickedHandler: function(pEvent) {
			...
			var sheetPlacement = info.getSheetsForJob(target.data('job'));
			...
			var infoPanel = $('<div>').addClass('jobinfo').append(sheetList);
		...
		getSelectedJob: function() {
			return this.element.find('tr.selected').data('job');
		},
